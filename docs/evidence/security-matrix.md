...
| S06 | Per-asset conservation; no unexplained mint, drain or subsidy | `batch.rs` (cross conservation), `settle_routed` pool reconciliation, `math.rs` checked totals, `packages/planner/src/costs.ts` exact allocation | `tests/security/properties.test.ts` (60 generated portfolios), `soak-properties.test.ts` (conservation after every lifecycle operation), `cost-allocation.test.ts` (allocation reconciles exactly), T09/T16 (vault = surplus, pool = 0) | model, local |
...
| S23 | Public demo operations have bounded resource use | `batch.rs` fixed maxima, `build-batch.ts` budget, `largest_remainder` caps, `services/api` request limits | `tests/program/atomic-batch.test.ts` (oversized body, batch size), `properties.test.ts` (band boundaries), `adversarial.test.ts` (max+1 refused by encoder and builder), `resource-limits.test.ts` (body, rate, concurrency, runner deadline), `soak-properties.test.ts` | unit, local |
