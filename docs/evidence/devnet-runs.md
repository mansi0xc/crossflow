...
| Config PDA | `7Fw4sjvYFCsdXPLFLqd5soPMnmKca2tojkgE2AVudnS2` |
...
- One explicit internal cross settled atomically for all three owners in a single transaction:
  ...
  the third owner's holdings were unchanged. The batch consumed 158,632 compute units and needed
  an address lookup table with 33 entries, because the three-owner instruction does not fit the
  legacy 1232-byte packet.
