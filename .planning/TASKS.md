...
User execution override: one task at a time; pause after its checks for user review. Use these states: NOT_STARTED → ACTIVE → READY_FOR_REVIEW → READY_FOR_USER_REVIEW → DONE; failure becomes BLOCKED or REWORK. T00 currently awaits user review with an open devnet funding prerequisite, not an unconditional PASS. Explicit user signoff/scope decision is required before the next task. Conditional tasks may become EXCLUDED with a dated reason. A dependency is satisfied only by DONE or an explicitly permitted exclusion that leaves the mandatory fallback intact. Never advance a money-path gate with an unresolved critical/high defect.
